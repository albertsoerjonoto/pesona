import type { Locale } from '@/lib/types';

type Translations = Record<string, Record<Locale, string>>;

export const translations: Translations = {
  // ── Navigation ──
  'nav.overview': { id: 'Home', en: 'Home' },
  'nav.log': { id: 'Rutinitas', en: 'Routine' },
  'nav.chat': { id: 'Coach', en: 'Coach' },
  'nav.profile': { id: 'Profil', en: 'Profile' },
  'nav.friends': { id: 'Progress', en: 'Progress' },

  // ── Common ──
  'common.save': { id: 'Simpan', en: 'Save' },
  'common.saving': { id: 'Menyimpan...', en: 'Saving...' },
  'common.saved': { id: 'Tersimpan!', en: 'Saved!' },
  'common.cancel': { id: 'Batal', en: 'Cancel' },
  'common.delete': { id: 'Hapus', en: 'Delete' },
  'common.add': { id: 'Tambah', en: 'Add' },
  'common.edit': { id: 'Edit', en: 'Edit' },
  'common.done': { id: 'Selesai', en: 'Done' },
  'common.back': { id: 'Kembali', en: 'Back' },
  'common.skip': { id: 'Lewati', en: 'Skip' },
  'common.change': { id: 'Ubah', en: 'Change' },
  'common.cal': { id: 'kal', en: 'cal' },
  'common.min': { id: 'mnt', en: 'min' },
  'common.noName': { id: 'Tanpa nama', en: 'No name' },
  'common.continue': { id: 'Lanjutkan', en: 'Continue' },
  'common.getStarted': { id: 'Mulai', en: 'Get Started' },
  'common.skipForNow': { id: 'Lewati dulu', en: 'Skip for now' },

  // ── Meals ──
  'meal.breakfast': { id: 'Sarapan', en: 'Breakfast' },
  'meal.lunch': { id: 'Makan Siang', en: 'Lunch' },
  'meal.dinner': { id: 'Makan Malam', en: 'Dinner' },
  'meal.other': { id: 'Lainnya', en: 'Other' },
  'meal.snack': { id: 'Camilan', en: 'Snack' },

  // ── Dashboard ──
  'dashboard.habits': { id: 'Kebiasaan', en: 'Habits' },
  'dashboard.diet': { id: 'Diet', en: 'Diet' },
  'dashboard.exercise': { id: 'Olahraga', en: 'Exercise' },
  'dashboard.addHabit': { id: 'Tambah Kebiasaan', en: 'Add Habit' },
  'dashboard.habitPlaceholder': { id: 'Nama kebiasaan', en: 'Habit name' },
  'dashboard.noHabits': { id: 'Belum ada kebiasaan. Ketuk Tambah untuk membuat!', en: 'No habits yet. Tap Add to create one!' },
  'dashboard.noExercise': { id: 'Belum ada olahraga hari ini', en: 'No exercise logged today' },
  'dashboard.eatenToday': { id: 'dimakan hari ini', en: 'eaten today' },
  'dashboard.addStatsHint': { id: 'Tambahkan data tubuh di Profil untuk melihat target kalori', en: 'Add your stats in Profile to see your calorie range' },
  'dashboard.only': { id: 'saja', en: 'only' },
  'dashboard.eaten': { id: 'Dimakan', en: 'Eaten' },
  'dashboard.burned': { id: 'Terbakar', en: 'Burned' },
  'dashboard.netCal': { id: 'Bersih', en: 'Net' },
  'dashboard.remaining': { id: 'tersisa', en: 'remaining' },
  'dashboard.over': { id: 'melebihi', en: 'over' },
  'dashboard.withinRange': { id: 'Dalam target', en: 'Within target' },
  'dashboard.belowRange': { id: 'di bawah target', en: 'below target' },
  'dashboard.aboveRange': { id: 'di atas target', en: 'above target' },
  'dashboard.target': { id: 'Target', en: 'Target' },
  'dashboard.failedLoadHabits': { id: 'Gagal memuat kebiasaan', en: 'Failed to load habits' },
  'dashboard.failedLoadFood': { id: 'Gagal memuat log makanan', en: 'Failed to load food logs' },
  'dashboard.failedLoadExercise': { id: 'Gagal memuat log olahraga', en: 'Failed to load exercise logs' },
  'dashboard.failedSaveOrder': { id: 'Gagal menyimpan urutan', en: 'Failed to save order' },
  'dashboard.failedUpdateHabit': { id: 'Gagal memperbarui kebiasaan', en: 'Failed to update habit' },
  'dashboard.failedAddHabit': { id: 'Gagal menambah kebiasaan', en: 'Failed to add habit' },
  'dashboard.failedDeleteHabit': { id: 'Gagal menghapus kebiasaan', en: 'Failed to delete habit' },
  'dashboard.privateHabit': { id: 'Privat (sembunyikan dari teman)', en: 'Private (hide from friends)' },
  'dashboard.streakInterval': { id: 'Interval streak', en: 'Streak interval' },
  'dashboard.streakIntervalDays': { id: 'hari', en: 'days' },
  'dashboard.streakIntervalHint': {
    id: 'Streak tetap berjalan selama kamu menyelesaikannya sekali dalam periode ini. Setiap kali selesai, hitungan mundur dimulai ulang.',
    en: 'Streak stays alive as long as you complete it at least once within this many days. Each completion resets the cooldown.',
  },

  // ── Nutrition labels ──
  'nutrition.protein': { id: 'Protein', en: 'Protein' },
  'nutrition.carbs': { id: 'Karbo', en: 'Carbs' },
  'nutrition.fat': { id: 'Lemak', en: 'Fat' },
  'nutrition.maintenance': { id: 'Pemeliharaan', en: 'Maintenance' },
  'nutrition.calDeficit': { id: 'kal defisit', en: 'cal deficit' },
  'nutrition.calSurplus': { id: 'kal surplus', en: 'cal surplus' },

  // ── Log page ──
  'log.title': { id: 'Log', en: 'Log' },
  'log.food': { id: '🍽️ Diet', en: '🍽️ Diet' },
  'log.exercise': { id: '🏃 Olahraga', en: '🏃 Exercise' },
  'log.addDiet': { id: 'Tambah Entri Diet', en: 'Add Diet Entry' },
  'log.editDiet': { id: 'Edit Entri Diet', en: 'Edit Diet Entry' },
  'log.addExercise': { id: 'Tambah Olahraga', en: 'Add Exercise' },
  'log.editExercise': { id: 'Edit Olahraga', en: 'Edit Exercise' },
  'log.whatDidYouEat': { id: 'Apa yang kamu makan?', en: 'What did you eat?' },
  'log.calories': { id: 'Kalori', en: 'Calories' },
  'log.proteinG': { id: 'Protein (g)', en: 'Protein (g)' },
  'log.carbsG': { id: 'Karbo (g)', en: 'Carbs (g)' },
  'log.fatG': { id: 'Lemak (g)', en: 'Fat (g)' },
  'log.exerciseType': { id: 'Jenis olahraga (cth: Lari)', en: 'Exercise type (e.g., Running)' },
  'log.durationMin': { id: 'Durasi (mnt)', en: 'Duration (min)' },
  'log.caloriesBurned': { id: 'Kalori terbakar', en: 'Calories burned' },
  'log.notesOptional': { id: 'Catatan (opsional)', en: 'Notes (optional)' },
  'log.noFoodLogged': { id: 'Belum ada makanan hari ini', en: 'No food logged today' },
  'log.noMealsLogged': { id: 'Belum ada makanan hari ini', en: 'No meals logged today' },
  'log.noSnacksLogged': { id: 'Belum ada camilan hari ini', en: 'No snacks logged today' },
  'log.noExerciseLogged': { id: 'Belum ada olahraga hari ini', en: 'No exercise logged today' },
  'log.deleteLog': { id: 'Hapus log', en: 'Delete log' },
  'log.deleteConfirm': { id: 'Yakin? Ini tidak bisa dibatalkan.', en: 'Are you sure? This cannot be undone.' },
  'log.calBurned': { id: 'kal terbakar', en: 'cal burned' },
  'log.protein': { id: 'protein', en: 'protein' },
  'log.carbs': { id: 'karbo', en: 'carbs' },
  'log.fat': { id: 'lemak', en: 'fat' },
  'log.failedCalories': { id: 'Kalori harus antara 0 dan 20.000', en: 'Calories must be between 0 and 20,000' },
  'log.failedMacros': { id: 'Makro harus antara 0 dan 5.000g', en: 'Macros must be between 0 and 5,000g' },
  'log.failedDuration': { id: 'Durasi harus antara 0 dan 1.440 menit', en: 'Duration must be between 0 and 1,440 minutes' },
  'log.failedCalBurned': { id: 'Kalori terbakar harus antara 0 dan 20.000', en: 'Calories burned must be between 0 and 20,000' },
  'log.failedSaveFood': { id: 'Gagal menyimpan log makanan', en: 'Failed to save food log' },
  'log.failedUpdateFood': { id: 'Gagal memperbarui log makanan', en: 'Failed to update food log' },
  'log.failedSaveExercise': { id: 'Gagal menyimpan log olahraga', en: 'Failed to save exercise log' },
  'log.failedUpdateExercise': { id: 'Gagal memperbarui log olahraga', en: 'Failed to update exercise log' },
  'log.failedDeleteFood': { id: 'Gagal menghapus log makanan', en: 'Failed to delete food log' },
  'log.failedDeleteExercise': { id: 'Gagal menghapus log olahraga', en: 'Failed to delete exercise log' },

  // ── Chat page ──
  'chat.title': { id: 'Chat', en: 'Chat' },
  'chat.welcome': {
    id: 'Halo! Saya bisa mencatat makanan & olahraga, mengedit catatan, menjawab pertanyaan nutrisi, dan memberi rekomendasi makanan. Coba "makan nasi goreng untuk makan siang", "ubah sarapan jadi 400 kal", atau "makan apa ya untuk makan malam?"',
    en: 'Hi! I can log food & exercise, edit your entries, answer nutrition questions, and give meal recommendations. Try "had nasi goreng for lunch", "change breakfast to 400 cal", or "what should I eat for dinner?"',
  },
  'chat.placeholder': { id: 'Tanya Sona soal skincare...', en: 'Ask Sona about skincare...' },
  'chat.placeholderPastDate': { id: 'Pindah ke hari ini untuk mengirim pesan', en: 'Switch to today to send messages' },
  'chat.saveToLog': { id: 'Simpan ke Log', en: 'Save to Log' },
  'chat.applyChanges': { id: 'Terapkan Perubahan', en: 'Apply Changes' },
  'chat.saveAndApply': { id: 'Simpan & Terapkan', en: 'Save & Apply Changes' },
  'chat.saved': { id: 'Tersimpan!', en: 'Saved!' },
  'chat.changesApplied': { id: 'Perubahan diterapkan!', en: 'Changes applied!' },
  'chat.couldntFind': { id: 'Saya tidak menemukan makanan atau olahraga di pesan kamu. Coba lebih spesifik!', en: "I couldn't find any food or exercise in your message. Try being more specific!" },
  'chat.messageTooLong': { id: 'Pesan terlalu panjang (maks 1.000 karakter)', en: 'Message too long (max 1,000 characters)' },
  'chat.failedSave': { id: 'Gagal menyimpan beberapa entri. Coba lagi.', en: 'Failed to save some entries. Please try again.' },
  'chat.failedApply': { id: 'Gagal menerapkan beberapa edit. Coba lagi.', en: 'Failed to apply some edits. Please try again.' },
  'chat.somethingWrong': { id: 'Maaf, terjadi kesalahan. Silakan coba lagi.', en: 'Sorry, something went wrong. Please try again.' },
  'chat.requestTimeout': { id: 'Permintaan terlalu lama. Silakan coba lagi.', en: 'Request timed out. Please try again.' },
  'chat.whatsInPhoto': { id: 'Apa isi foto ini?', en: "What's in this photo?" },
  'chat.found': { id: 'Ditemukan', en: 'Found' },
  'chat.foodItems': { id: 'item makanan', en: 'food item(s)' },
  'chat.exercises': { id: 'olahraga', en: 'exercise(s)' },
  'chat.foodEdits': { id: 'edit makanan', en: 'food edit(s)' },
  'chat.exerciseEdits': { id: 'edit olahraga', en: 'exercise edit(s)' },
  'chat.reviewAndTap': { id: 'Periksa lalu ketuk', en: 'Review and tap' },
  'chat.toConfirm': { id: 'untuk konfirmasi.', en: 'to confirm.' },
  'chat.and': { id: 'dan', en: 'and' },
  'chat.viewingHistory': { id: 'Melihat riwayat chat untuk {date}', en: 'Viewing chat history for {date}' },
  'chat.noHistory': { id: 'Tidak ada chat pada tanggal ini', en: 'No chat messages on this date' },

  // ── Profile page ──
  'profile.title': { id: 'Profil', en: 'Profile' },
  'profile.account': { id: 'Akun', en: 'Account' },
  'profile.displayName': { id: 'Nama Tampilan', en: 'Display Name' },
  'profile.displayNamePlaceholder': { id: 'Nama kamu', en: 'Your name' },
  'profile.username': { id: 'Username', en: 'Username' },
  'profile.usernamePlaceholder': { id: 'username_kamu', en: 'your_username' },
  'profile.usernameHint': { id: '3-20 karakter, huruf, angka, underscore', en: '3-20 chars, letters, numbers, underscores' },
  'profile.usernameTaken': { id: 'Username sudah digunakan', en: 'Username already taken' },
  'profile.email': { id: 'Email', en: 'Email' },
  'profile.calorieTarget': { id: 'Target Kalori', en: 'Calorie Target' },
  'profile.deficit': { id: 'Defisit', en: 'Deficit' },
  'profile.maintenance': { id: 'Stabil', en: 'Maint.' },
  'profile.surplus': { id: 'Surplus', en: 'Surplus' },
  'profile.belowTdee': { id: 'kal di bawah TDEE', en: 'cal below TDEE' },
  'profile.aboveTdee': { id: 'kal di atas TDEE', en: 'cal above TDEE' },
  'profile.bodyStats': { id: 'Data Tubuh', en: 'Body Stats' },
  'profile.dob': { id: 'Tanggal Lahir', en: 'Date of Birth' },
  'profile.gender': { id: 'Jenis Kelamin', en: 'Gender' },
  'profile.male': { id: 'Pria', en: 'Male' },
  'profile.female': { id: 'Wanita', en: 'Female' },
  'profile.height': { id: 'Tinggi', en: 'Height' },
  'profile.weight': { id: 'Berat', en: 'Weight' },
  'profile.language': { id: 'Bahasa', en: 'Language' },
  'profile.display': { id: 'Tampilan', en: 'Display' },
  'profile.desktopLayout': { id: 'Tata Letak Desktop', en: 'Desktop Layout' },
  'profile.compact': { id: 'Kompak', en: 'Compact' },
  'profile.expanded': { id: 'Lebar', en: 'Expanded' },
  'profile.saveChanges': { id: 'Simpan Perubahan', en: 'Save Changes' },
  'profile.signOut': { id: 'Keluar', en: 'Sign Out' },
  'profile.deleteAccount': { id: 'Hapus Akun', en: 'Delete Account' },
  'profile.deleteAccountMsg': {
    id: 'Ini akan menghapus akun dan semua data kamu secara permanen (kebiasaan, log makanan, log olahraga). Tidak bisa dibatalkan.',
    en: 'This will permanently delete your account and all your data (habits, food logs, exercise logs). This cannot be undone.',
  },
  'profile.deleting': { id: 'Menghapus...', en: 'Deleting...' },
  'profile.savedSuccess': { id: 'Profil tersimpan!', en: 'Profile saved!' },
  'profile.failedSave': { id: 'Gagal menyimpan. Silakan coba lagi.', en: 'Failed to save. Please try again.' },
  'profile.failedCreate': { id: 'Gagal membuat profil', en: 'Failed to create profile' },
  'profile.photoUpdated': { id: 'Foto diperbarui!', en: 'Photo updated!' },
  'profile.failedUpload': { id: 'Gagal mengunggah foto', en: 'Failed to upload photo' },
  'profile.failedDelete': { id: 'Gagal menghapus akun. Silakan coba lagi.', en: 'Failed to delete account. Please try again.' },
  'profile.to': { id: 'sampai', en: 'to' },
  'profile.calOfTdee': { id: 'kal dari TDEE', en: 'cal of TDEE' },
  'profile.rangeError': { id: 'Min harus ≤ maks', en: 'Min must be ≤ max' },
  'profile.calorieError': { id: 'Harus antara 1 dan 2.000', en: 'Must be between 1 and 2,000' },
  'profile.dobError': { id: 'Harus tanggal yang valid (usia 1-120)', en: 'Must be a valid past date (age 1–120)' },
  'profile.heightError': { id: 'Harus antara 50 dan 300 cm', en: 'Must be between 50 and 300 cm' },
  'profile.weightError': { id: 'Harus antara 10 dan 500 kg', en: 'Must be between 10 and 500 kg' },

  // ── Auth pages ──
  'auth.appName': { id: 'Pesona.io', en: 'Pesona.io' },
  'auth.loginSubtitle': { id: 'Jadi versi paling menarik dari dirimu', en: 'Become the most attractive version of yourself' },
  'auth.signupSubtitle': { id: 'Jadi versi paling menarik dari dirimu', en: 'Become the most attractive version of yourself' },
  'auth.loginSubtagline': { id: 'AI beauty & body coach untuk kamu', en: 'Your AI beauty & body coach' },
  'auth.usernameOrEmail': { id: 'Username atau email', en: 'Username or email' },
  'login.usernamePlaceholder': { id: 'username atau email', en: 'username or email' },
  'auth.password': { id: 'Password', en: 'Password' },
  'auth.confirmPassword': { id: 'Konfirmasi Password', en: 'Confirm Password' },
  'auth.signIn': { id: 'Masuk', en: 'Sign In' },
  'auth.signingIn': { id: 'Masuk...', en: 'Signing in...' },
  'auth.createAccount': { id: 'Buat Akun', en: 'Create Account' },
  'auth.creatingAccount': { id: 'Membuat akun...', en: 'Creating account...' },
  'auth.dontHaveAccount': { id: 'Belum punya akun?', en: "Don't have an account?" },
  'auth.signUp': { id: 'Daftar dong', en: 'Sign up' },
  'auth.alreadyHaveAccount': { id: 'Sudah punya akun?', en: 'Already have an account?' },
  'auth.signInLink': { id: 'Masuk', en: 'Sign in' },
  'auth.username': { id: 'Username', en: 'Username' },
  'auth.email': { id: 'Email', en: 'Email' },
  'auth.usernameRequirements': { id: '3-20 karakter, huruf, angka, underscore', en: '3-20 characters, letters, numbers, underscores' },
  'auth.usernameInvalid': { id: 'Username harus 3-20 karakter (huruf, angka, underscore)', en: 'Username must be 3-20 characters (letters, numbers, underscores)' },
  'auth.passwordMinLength': { id: 'Password minimal 6 karakter', en: 'Password must be at least 6 characters' },
  'auth.passwordsNoMatch': { id: 'Password tidak cocok', en: 'Passwords do not match' },
  'auth.userNotFound': { id: 'Username tidak ditemukan', en: 'Username not found' },
  'auth.somethingWrong': { id: 'Terjadi kesalahan. Silakan coba lagi.', en: 'Something went wrong. Please try again.' },
  'auth.emailConfirmFailed': { id: 'Konfirmasi email gagal. Silakan coba daftar lagi.', en: 'Email confirmation failed. Please try signing up again.' },
  'auth.checkEmail': { id: 'Periksa email kamu', en: 'Check your email' },
  'auth.checkEmailDesc': { id: 'Kami mengirim link konfirmasi ke', en: 'We sent a confirmation link to' },
  'auth.checkEmailAction': { id: 'Klik untuk mengaktifkan akun kamu.', en: 'Click it to activate your account.' },
  'auth.checkSpam': { id: 'Tidak menerima? Periksa folder spam.', en: "Didn't get it? Check your spam folder." },
  'auth.failedCreate': { id: 'Gagal membuat akun', en: 'Failed to create account' },
  'auth.signupWithEmail': { id: 'Daftar dengan Email', en: 'Sign up with Email' },
  'auth.signupWithApple': { id: 'Daftar dengan Apple', en: 'Sign up with Apple' },
  'auth.signupWithGoogle': { id: 'Daftar dengan Google', en: 'Sign up with Google' },
  'auth.loginWithEmail': { id: 'Masuk dengan Email', en: 'Sign in with Email' },
  'auth.loginWithApple': { id: 'Masuk dengan Apple', en: 'Sign in with Apple' },
  'auth.loginWithGoogle': { id: 'Masuk dengan Google', en: 'Sign in with Google' },
  'auth.back': { id: 'Kembali', en: 'Back' },

  // ── Onboarding ──
  'onboarding.nameTitle': { id: 'Siapa nama kamu?', en: 'What should we call you?' },
  'onboarding.nameSubtitle': { id: 'Kamu bisa mengubahnya nanti', en: 'You can always change this later' },
  'onboarding.genderTitle': { id: 'Apa jenis kelamin kamu?', en: "What's your gender?" },
  'onboarding.genderSubtitle': { id: 'Digunakan untuk menghitung kebutuhan nutrisi harian', en: 'Used to calculate your daily nutrition needs' },
  'onboarding.dobTitle': { id: 'Kapan kamu lahir?', en: 'When were you born?' },
  'onboarding.dobSubtitle': { id: 'Membantu memperkirakan kebutuhan kalori', en: 'Helps us estimate your calorie needs' },
  'onboarding.bodyTitle': { id: 'Ukuran tubuh kamu', en: 'Your body measurements' },
  'onboarding.bodySubtitle': { id: 'Untuk rekomendasi kalori yang lebih akurat', en: 'For more accurate calorie recommendations' },
  'onboarding.male': { id: '👨 Pria', en: '👨 Male' },
  'onboarding.female': { id: '👩 Wanita', en: '👩 Female' },
  'onboarding.yourName': { id: 'Nama kamu', en: 'Your name' },
  'onboarding.height': { id: 'Tinggi', en: 'Height' },
  'onboarding.weight': { id: 'Berat', en: 'Weight' },
  'onboarding.failedSave': { id: 'Gagal menyimpan. Silakan coba lagi.', en: 'Failed to save. Please try again.' },
  'onboarding.invalidDob': { id: 'Masukkan tanggal lahir yang valid', en: 'Please enter a valid date of birth' },
  'onboarding.heightRange': { id: 'Tinggi harus antara 50 dan 300 cm', en: 'Height must be between 50 and 300 cm' },
  'onboarding.weightRange': { id: 'Berat harus antara 10 dan 500 kg', en: 'Weight must be between 10 and 500 kg' },

  // ── Date navigation ──
  'date.day': { id: 'Hari', en: 'Day' },
  'date.week': { id: 'Minggu', en: 'Week' },
  'date.month': { id: 'Bulan', en: 'Month' },
  'date.year': { id: 'Tahun', en: 'Year' },
  'date.today': { id: 'Hari Ini', en: 'Today' },
  'date.yesterday': { id: 'Kemarin', en: 'Yesterday' },
  'date.tomorrow': { id: 'Besok', en: 'Tomorrow' },
  'date.thisWeek': { id: 'Minggu Ini', en: 'This Week' },
  'date.lastWeek': { id: 'Minggu Lalu', en: 'Last Week' },
  'date.nextWeek': { id: 'Minggu Depan', en: 'Next Week' },
  'date.thisMonth': { id: 'Bulan Ini', en: 'This Month' },
  'date.lastMonth': { id: 'Bulan Lalu', en: 'Last Month' },

  // ── DOB Picker breadcrumbs ──
  'dob.year': { id: 'Tahun', en: 'Year' },
  'dob.month': { id: 'Bulan', en: 'Month' },
  'dob.day': { id: 'Hari', en: 'Day' },

  // ── Months ──
  'month.1': { id: 'Januari', en: 'January' },
  'month.2': { id: 'Februari', en: 'February' },
  'month.3': { id: 'Maret', en: 'March' },
  'month.4': { id: 'April', en: 'April' },
  'month.5': { id: 'Mei', en: 'May' },
  'month.6': { id: 'Juni', en: 'June' },
  'month.7': { id: 'Juli', en: 'July' },
  'month.8': { id: 'Agustus', en: 'August' },
  'month.9': { id: 'September', en: 'September' },
  'month.10': { id: 'Oktober', en: 'October' },
  'month.11': { id: 'November', en: 'November' },
  'month.12': { id: 'Desember', en: 'December' },

  // ── Drinks ──
  'drink.water': { id: 'Air', en: 'Water' },
  'drink.coffee': { id: 'Kopi', en: 'Coffee' },
  'drink.tea': { id: 'Teh', en: 'Tea' },
  'drink.juice': { id: 'Jus', en: 'Juice' },
  'drink.soda': { id: 'Soda', en: 'Soda' },
  'drink.milk': { id: 'Susu', en: 'Milk' },
  'drink.smoothie': { id: 'Smoothie', en: 'Smoothie' },
  'drink.other': { id: 'Lainnya', en: 'Other' },
  'drink.drinks': { id: 'Minuman', en: 'Drinks' },
  'drink.addDrink': { id: 'Tambah Minuman', en: 'Add Drink' },
  'drink.editDrink': { id: 'Edit Minuman', en: 'Edit Drink' },
  'drink.whatDidYouDrink': { id: 'Apa yang kamu minum?', en: 'What did you drink?' },
  'drink.volumeMl': { id: 'Volume (ml)', en: 'Volume (ml)' },
  'drink.noDrinksLogged': { id: 'Belum ada minuman hari ini', en: 'No drinks logged today' },
  'drink.failedSave': { id: 'Gagal menyimpan log minuman', en: 'Failed to save drink log' },
  'drink.failedUpdate': { id: 'Gagal memperbarui log minuman', en: 'Failed to update drink log' },
  'drink.failedDelete': { id: 'Gagal menghapus log minuman', en: 'Failed to delete drink log' },
  'drink.failedVolume': { id: 'Volume harus antara 0 dan 10.000 ml', en: 'Volume must be between 0 and 10,000 ml' },

  // ── Dashboard drinks ──
  'dashboard.food': { id: 'Makanan', en: 'Food' },
  'dashboard.breakfast': { id: 'Sarapan', en: 'Bfast' },
  'dashboard.lunch': { id: 'Siang', en: 'Lunch' },
  'dashboard.dinner': { id: 'Malam', en: 'Dinner' },
  'dashboard.snacks': { id: 'Camilan', en: 'Snacks' },
  'dashboard.drinks': { id: 'Minuman', en: 'Drinks' },
  'dashboard.failedLoadDrinks': { id: 'Gagal memuat log minuman', en: 'Failed to load drink logs' },

  // ── Log page drinks ──
  'log.meals': { id: 'Makanan', en: 'Meals' },
  'log.snacks': { id: 'Camilan', en: 'Snacks' },
  'log.drinks': { id: 'Minuman', en: 'Drinks' },

  // ── Chat drinks ──
  'chat.drinkItems': { id: 'item minuman', en: 'drink(s)' },
  'chat.drinkEdits': { id: 'edit minuman', en: 'drink edit(s)' },

  // ── Profile water goal ──
  'profile.waterGoal': { id: 'Target Air', en: 'Water Goal' },
  'profile.waterGoalUnit': { id: 'ml per hari', en: 'ml per day' },

  // ── Emoji picker categories ──
  'emoji.fitness': { id: 'Kebugaran', en: 'Fitness' },
  'emoji.health': { id: 'Kesehatan', en: 'Health' },
  'emoji.food': { id: 'Makanan', en: 'Food' },
  'emoji.mind': { id: 'Pikiran', en: 'Mind' },
  'emoji.productivity': { id: 'Produktivitas', en: 'Productivity' },
  'emoji.social': { id: 'Sosial', en: 'Social' },
  'emoji.nature': { id: 'Alam', en: 'Nature' },
  'emoji.faces': { id: 'Wajah', en: 'Faces' },
  'emoji.hands': { id: 'Tangan', en: 'Hands' },
  'emoji.symbols': { id: 'Simbol', en: 'Symbols' },
  'emoji.objects': { id: 'Benda', en: 'Objects' },
  'emoji.travel': { id: 'Perjalanan', en: 'Travel' },
  'emoji.flags': { id: 'Bendera', en: 'Flags' },

  // ── Log page habits tab ──
  'log.habits': { id: '✅ Kebiasaan', en: '✅ Habits' },
  'log.noHabitsLogged': { id: 'Belum ada kebiasaan. Tambah di Dashboard!', en: 'No habits yet. Add them on Dashboard!' },

  // ── Log page measurements tab ──
  'log.measurements': { id: '📏 Ukuran', en: '📏 Measurements' },
  'log.addMeasurement': { id: 'Tambah Pengukuran', en: 'Add Measurement' },
  'log.editMeasurement': { id: 'Edit Pengukuran', en: 'Edit Measurement' },
  'log.noMeasurements': { id: 'Belum ada pengukuran hari ini', en: 'No measurements logged today' },
  'log.heightCm': { id: 'Tinggi (cm)', en: 'Height (cm)' },
  'log.weightKg': { id: 'Berat (kg)', en: 'Weight (kg)' },
  'log.lastMeasurement': { id: 'Terakhir', en: 'Last' },
  'log.on': { id: 'pada', en: 'on' },
  'log.failedSaveMeasurement': { id: 'Gagal menyimpan pengukuran', en: 'Failed to save measurement' },
  'log.failedUpdateMeasurement': { id: 'Gagal memperbarui pengukuran', en: 'Failed to update measurement' },
  'log.failedDeleteMeasurement': { id: 'Gagal menghapus pengukuran', en: 'Failed to delete measurement' },
  'log.failedLoadMeasurements': { id: 'Gagal memuat pengukuran', en: 'Failed to load measurements' },
  'log.failedLoadHabits': { id: 'Gagal memuat kebiasaan', en: 'Failed to load habits' },
  'log.heightOrWeightRequired': { id: 'Tinggi atau berat harus diisi', en: 'Height or weight is required' },
  'log.loggedAt': { id: 'Waktu', en: 'Time' },

  // ── Chat measurements ──
  'chat.measurementItems': { id: 'item pengukuran', en: 'measurement(s)' },
  'chat.measurementEdits': { id: 'edit pengukuran', en: 'measurement edit(s)' },

  // ── Friends ──
  'friends.title': { id: 'Teman', en: 'Friends' },
  'friends.feed': { id: 'Feed', en: 'Feed' },
  'friends.friendsList': { id: 'Teman', en: 'Friends' },
  'friends.addFriend': { id: 'Tambah', en: 'Add' },
  'friends.searchPlaceholder': { id: 'Cari nama, username, atau email...', en: 'Search name, username, or email...' },
  'friends.sendRequest': { id: 'Tambah Teman', en: 'Add Friend' },
  'friends.pending': { id: 'Menunggu...', en: 'Pending...' },
  'friends.alreadyFriends': { id: 'Teman ✓', en: 'Friends ✓' },
  'friends.accept': { id: 'Terima', en: 'Accept' },
  'friends.decline': { id: 'Tolak', en: 'Decline' },
  'friends.cancelRequest': { id: 'Batalkan', en: 'Cancel' },
  'friends.unfriend': { id: 'Hapus Teman', en: 'Unfriend' },
  'friends.noFriendsYet': { id: 'Belum punya teman', en: 'No friends yet' },
  'friends.addFriendsHint': { id: 'Tambah teman untuk melihat aktivitas mereka!', en: 'Add friends to see their activity!' },
  'friends.noActivity': { id: 'Belum ada aktivitas hari ini', en: 'No activity today' },
  'friends.noResults': { id: 'Tidak ditemukan', en: 'No results found' },
  'friends.pendingRequests': { id: 'Permintaan Masuk', en: 'Incoming Requests' },
  'friends.sentRequests': { id: 'Permintaan Terkirim', en: 'Sent Requests' },
  'friends.failedLoad': { id: 'Gagal memuat data teman', en: 'Failed to load friends data' },
  'friends.failedSendRequest': { id: 'Gagal mengirim permintaan', en: 'Failed to send request' },
  'friends.failedAccept': { id: 'Gagal menerima permintaan', en: 'Failed to accept request' },
  'friends.failedDecline': { id: 'Gagal menolak permintaan', en: 'Failed to decline request' },
  'friends.failedUnfriend': { id: 'Gagal menghapus teman', en: 'Failed to unfriend' },
  'friends.failedSearch': { id: 'Gagal mencari pengguna', en: 'Failed to search users' },
  'friends.requestSent': { id: 'Permintaan terkirim!', en: 'Request sent!' },
  'friends.requestAccepted': { id: 'Permintaan diterima!', en: 'Request accepted!' },
  'friends.unfriended': { id: 'Teman dihapus', en: 'Unfriended' },
  'friends.unfriendConfirm': { id: 'Yakin ingin menghapus {name} dari daftar teman?', en: 'Are you sure you want to unfriend {name}?' },
  'friends.sharedHabits': { id: 'Undangan Kebiasaan', en: 'Habit Invitations' },
  'friends.sharedHabitInvite': { id: '{name} mengajak kamu untuk berbagi kebiasaan', en: '{name} wants to share a habit with you' },
  'friends.noSharedInvites': { id: 'Tidak ada undangan kebiasaan', en: 'No habit invitations' },
  'friends.shareHabit': { id: 'Bagikan ke Teman', en: 'Share with Friend' },
  'friends.selectFriend': { id: 'Pilih teman', en: 'Select a friend' },
  'friends.habitShared': { id: 'Kebiasaan dibagikan!', en: 'Habit shared!' },
  'friends.failedShare': { id: 'Gagal membagikan kebiasaan', en: 'Failed to share habit' },
  'friends.sharedAccepted': { id: 'Undangan diterima!', en: 'Invitation accepted!' },
  'friends.failedAcceptShared': { id: 'Gagal menerima undangan', en: 'Failed to accept invitation' },
  'friends.failedRejectShared': { id: 'Gagal menolak undangan', en: 'Failed to reject invitation' },
  'friends.feedAll': { id: 'Semua', en: 'All' },
  'friends.feedMine': { id: 'Aktivitas Saya', en: 'My Activity' },
  'friends.feedFriends': { id: 'Teman', en: 'Friends' },
  'friends.loadMore': { id: 'Muat Lebih Banyak', en: 'Load More' },
  'friends.noFeedYet': { id: 'Belum ada aktivitas. Tambah teman dan mulai lacak kebiasaan bersama!', en: 'No activity yet. Add friends and start tracking habits together!' },
  'friends.completed': { id: 'menyelesaikan', en: 'completed' },
  'friends.hitStreak': { id: 'mencapai streak', en: 'hit a' },
  'friends.dayStreak': { id: 'hari streak pada', en: 'day streak on' },
  'friends.nowFriends': { id: 'sekarang berteman', en: 'are now friends' },
  'friends.sharedStreak': { id: 'sedang streak bersama', en: 'are on a streak together on' },
  'friends.ago': { id: 'lalu', en: 'ago' },
  'friends.justNow': { id: 'Baru saja', en: 'Just now' },
  'friends.minutesAgo': { id: 'mnt lalu', en: 'min ago' },
  'friends.hoursAgo': { id: 'jam lalu', en: 'hr ago' },
  'friends.daysAgo': { id: 'hari lalu', en: 'd ago' },
  'friends.food': { id: 'Makan', en: 'Ate' },
  'friends.exercise': { id: 'Olahraga', en: 'Exercised' },
  'friends.drink': { id: 'Minum', en: 'Drank' },
  'friends.habit': { id: 'Selesai', en: 'Completed' },
  'friends.exerciseCompleted': { id: 'olahraga', en: 'exercised' },
  'friends.exerciseFor': { id: 'selama', en: 'for' },
  'friends.exerciseMin': { id: 'mnt', en: 'min' },
  'friends.calorieGoalMet': { id: 'mencapai target kalori', en: 'hit calorie goal' },
  'friends.proteinGoalMet': { id: 'mencapai target protein', en: 'hit protein goal' },
  'friends.fatGoalMet': { id: 'mencapai target lemak', en: 'hit fat goal' },
  'friends.carbsGoalMet': { id: 'mencapai target karbohidrat', en: 'hit carbs goal' },
  'friends.waterGoalMet': { id: 'mencapai target air', en: 'hit water goal' },
  'friends.hitGoals': { id: 'mencapai', en: 'hit' },
  'friends.goals': { id: 'target', en: 'goals' },
  'friends.deleteFeedEvent': { id: 'Hapus', en: 'Delete' },
  'friends.deleteConfirmTitle': { id: 'Hapus notifikasi?', en: 'Delete notification?' },
  'friends.deleteConfirmMessage': { id: 'Notifikasi ini akan dihapus dari feed.', en: 'This notification will be removed from the feed.' },
  'friends.feedDeleted': { id: 'Notifikasi dihapus', en: 'Notification deleted' },
  'friends.feedDeleteFailed': { id: 'Gagal menghapus notifikasi', en: 'Failed to delete notification' },
  'friends.reject': { id: 'Tolak', en: 'Reject' },

  // ── Analytics ──
  'analytics.habitStreak': { id: 'Streak Kebiasaan', en: 'Habit Streak' },
  'analytics.habitStreakDesc': { id: 'hari semua kebiasaan selesai berturut-turut', en: 'consecutive days all habits completed' },
  'analytics.bestStreak': { id: 'Rekor terbaik', en: 'Best streak' },
  'analytics.days': { id: 'hari', en: 'days' },
  'analytics.completionRate': { id: 'Selesai periode ini', en: 'Done this period' },
  'analytics.habitCompletion': { id: 'Kebiasaan Harian', en: 'Daily Habits' },
  'analytics.habitsCompleted': { id: 'kebiasaan selesai', en: 'habits done' },
  'analytics.less': { id: 'Sedikit', en: 'Less' },
  'analytics.more': { id: 'Banyak', en: 'More' },
  'analytics.calorieIntake': { id: 'Kalori Dimakan per Hari', en: 'Daily Calorie Intake' },
  'analytics.avg': { id: 'Rata-rata', en: 'Avg' },
  'analytics.day': { id: 'hari', en: 'day' },
  'analytics.perDay': { id: '/hari', en: '/day' },
  'analytics.daysOnTarget': { id: 'hari sesuai target', en: 'days on target' },
  'analytics.macroSplit': { id: 'Rata-rata Makro Harian', en: 'Daily Macro Average' },
  'analytics.noData': { id: 'Tidak ada data untuk periode ini', en: 'No data for this period' },
  'analytics.daysActive': { id: 'hari aktif', en: 'days active' },
  'analytics.waterIntake': { id: 'Asupan Air Harian', en: 'Daily Water Intake' },
  'analytics.daysGoalMet': { id: 'hari target tercapai', en: 'days goal met' },
  'analytics.weightTrend': { id: 'Tren Berat Badan', en: 'Weight Trend' },
  'analytics.vsLastWeek': { id: 'dibanding minggu lalu', en: 'compared to last week' },
  'analytics.vsLastMonth': { id: 'dibanding bulan lalu', en: 'compared to last month' },
  'analytics.thisWeek': { id: 'Minggu ini', en: 'This week' },
  'analytics.thisMonth': { id: 'Bulan ini', en: 'This month' },
  'analytics.thisYear': { id: 'Tahun ini', en: 'This year' },
  'analytics.avgDailyIntake': { id: 'Rata-rata kalori dimakan/hari', en: 'Avg daily calorie intake' },
  'analytics.habitCompletionRate': { id: 'Kebiasaan selesai', en: 'Habits completed' },
  'analytics.activeDays': { id: 'Hari ada olahraga', en: 'Days with exercise' },
  'analytics.perHabit': { id: 'Per Kebiasaan', en: 'Per Habit' },
  'analytics.timesCompleted': { id: 'kali selesai', en: 'times done' },
  'analytics.of': { id: 'dari', en: 'of' },

  // ── Voice input ──
  'voice.micPermissionDenied': { id: 'Akses mikrofon ditolak', en: 'Microphone access denied' },
  'voice.noSpeechDetected': { id: 'Tidak ada suara terdeteksi', en: 'No speech detected' },
  'voice.transcriptionFailed': { id: 'Gagal menyalin suara', en: 'Transcription failed' },
  'voice.networkError': { id: 'Kesalahan jaringan saat input suara', en: 'Network error during voice input' },

  // ── Tour ──
  'tour.welcomeTitle': { id: 'Asisten AI kamu', en: 'Your AI Assistant' },
  'tour.welcomeDesc': {
    id: 'Ini asisten AI kamu. Dia bisa mencatat makanan, minuman, dan olahraga cukup dengan ngobrol. Yuk coba!',
    en: "This is your AI assistant. It can log food, drinks, and exercise just by chatting naturally. Let's try it!",
  },
  'tour.inputTitle': { id: 'Coba ketik di sini', en: 'Try typing here' },
  'tour.inputDesc': {
    id: 'Ketik apa yang kamu makan atau minum hari ini — misalnya "nasi goreng untuk makan siang" atau "minum segelas air". Kamu juga bisa pakai mikrofon atau kamera!',
    en: 'Type what you ate or drank today — like "nasi goreng for lunch" or "drank a glass of water". You can also tap the mic to speak or the camera to take a photo!',
  },
  'tour.waitingTitle': { id: 'AI sedang menganalisis...', en: 'AI is analyzing...' },
  'tour.waitingDesc': {
    id: 'Bagus! AI sedang menganalisis inputmu...',
    en: 'Great! The AI is analyzing your input...',
  },
  'tour.resultTitle': { id: 'Hasil AI', en: 'AI Results' },
  'tour.resultDesc': {
    id: 'Ini yang ditemukan AI! Kamu bisa ketuk untuk edit detail sebelum menyimpan. Kalau sudah siap, ketuk Simpan.',
    en: "Here's what the AI found! You can tap to edit the details before saving. When you're ready, tap Save to log it.",
  },
  'tour.resultFallback': {
    id: 'Hmm, itu tidak menghasilkan log. Coba sesuatu seperti "makan nasi ayam untuk makan siang". Atau lewati untuk lanjut tur.',
    en: 'Hmm, that didn\'t create a log entry. Try something like "I had rice and chicken for lunch". Or skip to continue the tour.',
  },
  'tour.logTitle': { id: 'Log kamu', en: 'Your Log' },
  'tour.logDesc': {
    id: 'Ini Log kamu. Semua makanan, camilan, minuman, dan olahraga hari ini ada di sini. Kamu juga bisa tambah entri manual dengan tombol Tambah.',
    en: 'This is your Log. All your meals, snacks, drinks, and exercises for today are here. You can also add entries manually with the Add button.',
  },
  'tour.dashboardTitle': { id: 'Ringkasan harian', en: 'Daily Overview' },
  'tour.dashboardDesc': {
    id: 'Dan ini Ringkasan kamu! Lihat bagaimana makananmu memengaruhi target kalori dan makro harian. AI menghitung semuanya untukmu. Kamu siap! 🎉',
    en: "And this is your Overview! See how your meal affects your daily calorie and macro targets. The AI calculates everything for you. You're all set! 🎉",
  },
  'tour.completeTitle': { id: 'Kamu siap! 💪', en: "You're ready! 💪" },
  'tour.completeDesc': {
    id: 'Kamu siap! Ingat, kamu bisa selalu ngobrol dengan AI untuk mencatat makanan, minuman, atau olahraga. Selamat menjalani hidup sehat!',
    en: "You're ready to go! Remember, you can always chat with the AI to log food, drinks, or exercise. Enjoy your health journey!",
  },
  'tour.next': { id: 'Lanjut', en: 'Next' },
  'tour.skipTour': { id: 'Lewati Tur', en: 'Skip Tour' },
  'tour.done': { id: 'Selesai!', en: 'Done!' },
  'tour.letsGo': { id: 'Ayo Mulai!', en: "Let's Go!" },
  'tour.apiError': {
    id: 'Ups, AI sedang bermasalah. Tidak apa — kamu bisa coba lagi kapan saja dari halaman Chat. Yuk lanjut tur!',
    en: "Oops, the AI is having trouble. No worries — you can try again anytime from the Chat page. Let's continue the tour!",
  },
  'tour.replayTour': { id: 'Ulangi Tur', en: 'Replay Tour' },
  'chat.welcomeTour': {
    id: 'Selamat datang, {name}! 👋 Yuk catat makanan atau minuman pertamamu. Hari ini kamu makan atau minum apa? Bisa ketik, pakai suara, atau foto!',
    en: "Welcome, {name}! 👋 Let's log your first meal together. What did you eat or drink today? You can type, use voice, or take a photo!",
  },

  // ── Skin Quiz Onboarding ──
  'skin.quiz.typeTitle': { id: 'Gimana kulit wajahmu biasanya?', en: "What's your skin usually like?" },
  'skin.quiz.typeSubtitle': { id: 'Pilih yang paling cocok dengan kondisi kulitmu', en: 'Pick the one that best matches your skin' },
  'skin.quiz.concernsTitle': { id: 'Apa yang paling mengganggu kulitmu?', en: "What bothers you most about your skin?" },
  'skin.quiz.concernsSubtitle': { id: 'Pilih satu atau lebih (min. 1)', en: 'Pick one or more (min. 1)' },
  'skin.quiz.goalsTitle': { id: 'Kulit impianmu kayak apa?', en: "What's your dream skin like?" },
  'skin.quiz.goalsSubtitle': { id: 'Pilih target kulitmu (min. 1)', en: 'Pick your skin goals (min. 1)' },
  'skin.quiz.budgetTitle': { id: 'Budget skincare bulananmu?', en: 'Your monthly skincare budget?' },
  'skin.quiz.budgetSubtitle': { id: 'Biar rekomendasi produk sesuai kantong', en: "So product recs fit your wallet" },
  'skin.quiz.extraTitle': { id: 'Sedikit info tambahan...', en: 'A little more about you...' },
  'skin.quiz.extraSubtitle': { id: 'Bantu Sona kasih saran yang lebih pas', en: "Help Sona give you better advice" },
  'skin.quiz.hijabLabel': { id: 'Aku memakai hijab', en: 'I wear hijab' },
  'skin.quiz.readyTitle': { id: 'Kamu siap! ✨', en: "You're all set! ✨" },
  'skin.quiz.readyDesc': { id: 'Sona akan buatkan skincare routine yang cocok untuk kamu berdasarkan jawaban di atas.', en: "Sona will create a skincare routine tailored to you based on your answers above." },
  'skin.quiz.startJourney': { id: 'Mulai Perjalanan Kulitku', en: 'Start My Skin Journey' },
  'skin.quiz.selectAtLeastOne': { id: 'Pilih minimal satu ya', en: 'Please select at least one' },

  // ── Skin Types ──
  'skin.type.oily': { id: 'Berminyak', en: 'Oily' },
  'skin.type.oily.desc': { id: 'Wajah sering terasa berminyak, terutama di T-zone', en: 'Face often feels oily, especially in the T-zone' },
  'skin.type.dry': { id: 'Kering', en: 'Dry' },
  'skin.type.dry.desc': { id: 'Kulit terasa kencang, kadang mengelupas', en: 'Skin feels tight, sometimes flaky' },
  'skin.type.combination': { id: 'Kombinasi', en: 'Combination' },
  'skin.type.combination.desc': { id: 'Berminyak di T-zone, kering di pipi', en: 'Oily in T-zone, dry on cheeks' },
  'skin.type.normal': { id: 'Normal', en: 'Normal' },
  'skin.type.normal.desc': { id: 'Tidak terlalu berminyak atau kering', en: 'Not too oily or dry' },
  'skin.type.sensitive': { id: 'Sensitif', en: 'Sensitive' },
  'skin.type.sensitive.desc': { id: 'Mudah merah, gatal, atau iritasi', en: 'Easily red, itchy, or irritated' },

  // ── Skin Concerns ──
  'skin.concern.acne': { id: 'Jerawat', en: 'Acne' },
  'skin.concern.dark_spots': { id: 'Flek hitam', en: 'Dark spots' },
  'skin.concern.dullness': { id: 'Kusam', en: 'Dullness' },
  'skin.concern.large_pores': { id: 'Pori-pori besar', en: 'Large pores' },
  'skin.concern.blackheads': { id: 'Komedo', en: 'Blackheads' },
  'skin.concern.redness': { id: 'Kemerahan', en: 'Redness' },
  'skin.concern.rough_texture': { id: 'Tekstur kasar', en: 'Rough texture' },
  'skin.concern.aging': { id: 'Tanda penuaan', en: 'Aging signs' },

  // ── Skin Goals ──
  'skin.goal.glowing': { id: 'Glowing & cerah', en: 'Glowing & bright' },
  'skin.goal.clear': { id: 'Bersih tanpa jerawat', en: 'Clear, acne-free' },
  'skin.goal.even_tone': { id: 'Merata tanpa flek', en: 'Even tone, no spots' },
  'skin.goal.hydrated': { id: 'Lembap & kenyal', en: 'Hydrated & bouncy' },
  'skin.goal.anti_aging': { id: 'Anti-aging', en: 'Anti-aging' },
  'skin.goal.small_pores': { id: 'Pori-pori kecil', en: 'Smaller pores' },

  // ── Budget ──
  'skin.budget.under_100k': { id: 'Di bawah Rp 100rb', en: 'Under Rp 100K' },
  'skin.budget.100k_300k': { id: 'Rp 100-300rb', en: 'Rp 100-300K' },
  'skin.budget.300k_500k': { id: 'Rp 300-500rb', en: 'Rp 300-500K' },
  'skin.budget.over_500k': { id: 'Di atas Rp 500rb', en: 'Over Rp 500K' },

  // ── Skin Feelings ──
  'skin.feeling.great': { id: 'Luar biasa', en: 'Amazing' },
  'skin.feeling.good': { id: 'Bagus', en: 'Good' },
  'skin.feeling.okay': { id: 'Biasa', en: 'Okay' },
  'skin.feeling.bad': { id: 'Kurang', en: 'Not great' },
  'skin.feeling.terrible': { id: 'Buruk', en: 'Terrible' },

  // ── Dashboard Skincare ──
  'dashboard.greeting.morning': { id: 'Selamat pagi', en: 'Good morning' },
  'dashboard.greeting.afternoon': { id: 'Selamat siang', en: 'Good afternoon' },
  'dashboard.greeting.evening': { id: 'Selamat malam', en: 'Good evening' },
  'dashboard.skinProfile': { id: 'Profil Kulit', en: 'Skin Profile' },
  'dashboard.startQuiz': { id: 'Mulai Skin Quiz', en: 'Start Skin Quiz' },
  'dashboard.editSkinProfile': { id: 'Edit Profil Kulit', en: 'Edit Skin Profile' },
  'dashboard.todayRoutine': { id: 'Rutinitas Hari Ini', en: "Today's Routine" },
  'dashboard.morningRoutine': { id: 'Routine Pagi', en: 'Morning Routine' },
  'dashboard.eveningRoutine': { id: 'Routine Malam', en: 'Evening Routine' },
  'dashboard.noRoutine': { id: 'Belum punya routine? Tanya Sona!', en: "No routine yet? Ask Sona!" },
  'dashboard.stepsCompleted': { id: 'langkah', en: 'steps' },
  'dashboard.dailyCheckin': { id: 'Gimana kulit kamu hari ini?', en: 'How is your skin today?' },
  'dashboard.tipOfDay': { id: 'Tips Hari Ini', en: 'Tip of the Day' },
  'dashboard.takePhoto': { id: 'Ambil foto progress', en: 'Take progress photo' },
  'dashboard.recentPhotos': { id: 'Foto Terbaru', en: 'Recent Photos' },
  'dashboard.viewAll': { id: 'Lihat Semua', en: 'View All' },
  'dashboard.streak': { id: 'hari', en: 'days' },

  // ── Routine Page ──
  'routine.morning': { id: 'Pagi', en: 'Morning' },
  'routine.evening': { id: 'Malam', en: 'Evening' },
  'routine.editRoutine': { id: 'Edit Routine', en: 'Edit Routine' },
  'routine.generateAI': { id: 'Generate dengan AI', en: 'Generate with AI' },
  'routine.completed': { id: 'Selesai!', en: 'Done!' },
  'routine.noRoutineYet': { id: 'Belum ada routine', en: 'No routine yet' },
  'routine.askSona': { id: 'Tanya Sona untuk buatkan routine', en: 'Ask Sona to create a routine' },

  // ── Chat / Coach (Skincare) ──
  'chat.thinking': { id: 'Sona sedang berpikir...', en: 'Sona is thinking...' },
  'chat.error': { id: 'Sona lagi sibuk, coba lagi ya', en: 'Sona is busy, try again' },
  'chat.chipRoutine': { id: 'Routine pagi', en: 'Morning routine' },
  'chat.chipSerum': { id: 'Rekomendasi serum', en: 'Serum recs' },
  'chat.chipOily': { id: 'Tips kulit berminyak', en: 'Oily skin tips' },
  'chat.chipProduct': { id: 'Cek produk', en: 'Check product' },
  'chat.chipPhoto': { id: 'Gimana kondisi foto terakhir aku?', en: 'How does my latest photo look?' },
  'chat.buyShopee': { id: 'Beli di Shopee', en: 'Buy on Shopee' },
  'chat.addToRoutine': { id: 'Tambah ke Routine', en: 'Add to Routine' },
  'chat.useRoutine': { id: 'Pakai Routine Ini', en: 'Use This Routine' },
  'chat.halal': { id: 'Halal', en: 'Halal' },
  'chat.bpom': { id: 'BPOM', en: 'BPOM' },

  // ── Photo Progress ──
  'progress.title': { id: 'Foto Progress', en: 'Photo Progress' },
  'progress.upload': { id: 'Upload Foto', en: 'Upload Photo' },
  'progress.compare': { id: 'Bandingkan', en: 'Compare' },
  'progress.noPhotos': { id: 'Belum ada foto progress', en: 'No progress photos yet' },
  'progress.takeFirst': { id: 'Ambil foto pertamamu untuk mulai tracking', en: 'Take your first photo to start tracking' },
  'progress.aiAnalysis': { id: 'Analisis AI', en: 'AI Analysis' },
  'progress.disclaimer': { id: 'Ini bukan diagnosis medis', en: 'This is not a medical diagnosis' },
  'progress.totalPhotos': { id: 'Total Foto', en: 'Total Photos' },
  'progress.weekStreak': { id: 'Minggu berturut-turut', en: 'Consecutive weeks' },

  // ── Products ──
  'product.halal': { id: 'Halal', en: 'Halal' },
  'product.bpom': { id: 'BPOM', en: 'BPOM' },
  'product.buyShopee': { id: 'Beli di Shopee', en: 'Buy on Shopee' },
  'product.ingredients': { id: 'Kandungan', en: 'Ingredients' },
  'product.title': { id: 'Produk Skincare', en: 'Skincare Products' },
  'product.compare': { id: 'Compare', en: 'Compare' },
  'product.searchPlaceholder': { id: 'Cari produk, brand, atau ingredient...', en: 'Search products, brands, or ingredients...' },
  'product.allCategories': { id: 'Semua', en: 'All' },
  'product.allSkin': { id: 'Semua Kulit', en: 'All Skin' },
  'product.resultsCount': { id: 'produk ditemukan', en: 'products found' },
  'product.noMatch': { id: 'Tidak ada produk yang cocok', en: 'No matching products' },
  'product.howToUse': { id: 'Cara Pakai', en: 'How to Use' },
  'product.suitableFor': { id: 'Cocok untuk', en: 'Suitable for' },
  'product.helpsFor': { id: 'Membantu', en: 'Helps with' },

  // ── Landing Page ──
  'landing.badge': { id: 'AI-Powered Beauty Coach', en: 'AI-Powered Beauty Coach' },
  'landing.heroTitle': { id: 'Jadi versi paling menarik dari', en: 'Become the most attractive version of' },
  'landing.heroTitleHighlight': { id: 'dirimu', en: 'yourself' },
  'landing.heroDesc': {
    id: 'Personal beauty & body coach yang paham kulit dan tubuh kamu. Skincare routine, progress tracking, dan saran personal — semua dalam Bahasa Indonesia.',
    en: 'Personal beauty & body coach that understands your skin and body. Skincare routines, progress tracking, and personalized advice — all in Bahasa Indonesia.',
  },
  'landing.ctaStart': { id: 'Mulai Sekarang', en: 'Get Started' },
  'landing.ctaSignup': { id: 'Daftar Gratis', en: 'Sign Up Free' },
  'landing.ctaLogin': { id: 'Masuk', en: 'Sign In' },
  'landing.priceFrom': { id: 'Mulai dari Rp 59.000/bulan', en: 'Starting from Rp 59,000/month' },
  'landing.whyPesona': { id: 'Kenapa Pesona?', en: 'Why Pesona?' },
  'landing.whyDesc': {
    id: 'Coach AI yang benar-benar paham kebutuhan kecantikan orang Indonesia.',
    en: 'An AI coach that truly understands Indonesian beauty needs.',
  },
  'landing.feature1Title': { id: 'Skincare Routine Personal', en: 'Personal Skincare Routine' },
  'landing.feature1Desc': {
    id: 'Rutinitas pagi & malam yang disesuaikan dengan jenis kulit, masalah, dan budget kamu.',
    en: 'Morning & evening routines customized to your skin type, concerns, and budget.',
  },
  'landing.feature2Title': { id: 'Foto Progress Tracking', en: 'Photo Progress Tracking' },
  'landing.feature2Desc': {
    id: 'Foto kulit kamu secara berkala dan lihat perubahan nyata dari waktu ke waktu.',
    en: 'Track your skin with regular photos and see real changes over time.',
  },
  'landing.feature3Title': { id: 'AI Coach Bahasa Indonesia', en: 'AI Coach in Bahasa Indonesia' },
  'landing.feature3Desc': {
    id: 'Tanya apa saja soal skincare dalam Bahasa Indonesia. Paham produk lokal dan iklim tropis.',
    en: 'Ask anything about skincare in Bahasa Indonesia. Understands local products and tropical climate.',
  },
  'landing.feature4Title': { id: 'Rekomendasi Produk Lokal', en: 'Local Product Recommendations' },
  'landing.feature4Desc': {
    id: 'Tau Skintific, Somethinc, Wardah, Glad2Glow, Kahf, dan produk Indonesia lainnya.',
    en: 'Knows Skintific, Somethinc, Wardah, Glad2Glow, Kahf, and other Indonesian products.',
  },
  'landing.feature5Title': { id: 'Paham Iklim Tropis', en: 'Tropical Climate Aware' },
  'landing.feature5Desc': {
    id: 'Saran yang cocok untuk cuaca panas & lembap Indonesia, termasuk tips saat puasa Ramadan.',
    en: 'Advice suited for Indonesia\'s hot & humid weather, including Ramadan fasting tips.',
  },
  'landing.feature6Title': { id: 'Skin Analysis dengan AI Vision', en: 'AI Vision Skin Analysis' },
  'landing.feature6Desc': {
    id: 'Upload foto wajah dan dapatkan analisis kondisi kulit serta rekomendasi perawatan.',
    en: 'Upload a face photo and get skin condition analysis with treatment recommendations.',
  },
  'landing.howItWorks': { id: 'Cara Kerja Pesona', en: 'How Pesona Works' },
  'landing.step1': { id: 'Jawab pertanyaan singkat tentang kulit kamu', en: 'Answer quick questions about your skin' },
  'landing.step2': { id: 'AI buatkan skincare routine personal', en: 'AI creates your personal skincare routine' },
  'landing.step3': { id: 'Tanya apa saja soal skincare ke AI coach', en: 'Ask the AI coach anything about skincare' },
  'landing.step4': { id: 'Foto dan pantau perubahan kulit kamu', en: 'Track and monitor your skin changes' },
  'landing.pricingTitle': { id: 'Mulai perjalanan kecantikanmu', en: 'Start your beauty journey' },
  'landing.pricingDesc': {
    id: 'Dapatkan skincare coach personal dengan harga kurang dari secangkir kopi per hari.',
    en: 'Get a personal skincare coach for less than a cup of coffee per day.',
  },
  'landing.pricingStartFrom': { id: 'Mulai dari', en: 'Starting from' },
  'landing.pricingPerMonth': { id: '/bulan', en: '/month' },
  'landing.pricingFeature1': { id: 'Skincare routine personal', en: 'Personal skincare routine' },
  'landing.pricingFeature2': { id: 'AI coach unlimited chat', en: 'Unlimited AI coach chat' },
  'landing.pricingFeature3': { id: 'Foto progress tracking', en: 'Photo progress tracking' },
  'landing.pricingFeature4': { id: 'Rekomendasi produk Indonesia', en: 'Indonesian product recommendations' },
  'landing.faqTitle': { id: 'FAQ', en: 'FAQ' },
  'landing.faq1q': { id: 'Apakah ini layanan medis?', en: 'Is this a medical service?' },
  'landing.faq1a': {
    id: 'Tidak. Pesona adalah produk wellness & coaching. Untuk masalah kulit serius, kami akan rekomendasikan kamu ke dokter kulit.',
    en: 'No. Pesona is a wellness & coaching product. For serious skin concerns, we\'ll recommend you see a dermatologist.',
  },
  'landing.faq2q': { id: 'Apakah produk yang direkomendasikan halal?', en: 'Are recommended products halal?' },
  'landing.faq2a': {
    id: 'Kami menandai status halal dan BPOM setiap produk. Kamu bisa filter hanya produk halal saja.',
    en: 'We mark halal and BPOM status for every product. You can filter to show only halal products.',
  },
  'landing.faq3q': { id: 'Berapa harganya?', en: 'How much does it cost?' },
  'landing.faq3a': {
    id: 'Mulai dari Rp 59.000/bulan untuk Plus. Ada juga tier Free dengan fitur terbatas.',
    en: 'Starting from Rp 59,000/month for Plus. There is also a Free tier with limited features.',
  },
  'landing.faq4q': { id: 'Apakah AI coach bisa berbahasa Inggris?', en: 'Can the AI coach speak English?' },
  'landing.faq4a': {
    id: 'Ya! Sona bisa bicara dalam Bahasa Indonesia dan English. Dia akan menyesuaikan dengan bahasa kamu.',
    en: 'Yes! Sona can speak both Bahasa Indonesia and English. She adapts to your language.',
  },
  'landing.disclaimer': {
    id: 'Pesona adalah produk wellness, bukan layanan medis. Untuk masalah kulit serius, konsultasikan ke dokter kulit.',
    en: 'Pesona is a wellness product, not a medical service. For serious skin concerns, consult a dermatologist.',
  },

  // ── Chat Welcome (hardcoded strings) ���─
  'chat.welcomeTitle': { id: 'Halo, aku Sona!', en: 'Hi, I\'m Sona!' },
  'chat.welcomeDesc': {
    id: 'AI beauty coach kamu dari Pesona.io. Aku bisa bantu:',
    en: 'Your AI beauty coach from Pesona.io. I can help with:',
  },
  'chat.welcomeRoutine': { id: 'Membuat skincare routine pagi & malam', en: 'Creating morning & evening skincare routines' },
  'chat.welcomeProduct': { id: 'Rekomendasi produk sesuai jenis kulit kamu', en: 'Product recommendations for your skin type' },
  'chat.welcomeAnalysis': { id: 'Analisis kondisi kulit dari foto', en: 'Analyzing skin condition from photos' },
  'chat.welcomeTips': { id: 'Tips perawatan untuk iklim tropis Indonesia', en: 'Skincare tips for tropical Indonesian climate' },
  'chat.routineMorning': { id: 'Routine Pagi', en: 'Morning Routine' },
  'chat.routineEvening': { id: 'Routine Malam', en: 'Evening Routine' },

  // ── Dashboard missing ──
  'dashboard.noSkinProfile': { id: 'Belum ada profil kulit', en: 'No skin profile yet' },
  'dashboard.loadError': { id: 'Gagal memuat data. Coba lagi.', en: 'Failed to load data. Try again.' },

  // ── Error & Not Found pages ──
  'error.title': { id: 'Terjadi kesalahan', en: 'Something went wrong' },
  'error.defaultMessage': { id: 'Terjadi kesalahan tak terduga. Silakan coba lagi.', en: 'An unexpected error occurred. Please try again.' },
  'error.retry': { id: 'Coba Lagi', en: 'Try Again' },
  'notFound.title': { id: 'Halaman tidak ditemukan', en: 'Page not found' },
  'notFound.message': { id: 'Halaman yang kamu cari tidak ada.', en: "The page you're looking for doesn't exist." },
  'notFound.goHome': { id: 'Ke Beranda', en: 'Go to Home' },

  // ── Products page remaining ──
  'product.category.all': { id: 'Semua', en: 'All' },
  'product.category.cleanser': { id: 'Cleanser', en: 'Cleanser' },
  'product.category.toner': { id: 'Toner', en: 'Toner' },
  'product.category.serum': { id: 'Serum', en: 'Serum' },
  'product.category.moisturizer': { id: 'Moisturizer', en: 'Moisturizer' },
  'product.category.sunscreen': { id: 'Sunscreen', en: 'Sunscreen' },
  'product.category.exfoliator': { id: 'Exfoliator', en: 'Exfoliator' },
  'product.category.mask': { id: 'Mask', en: 'Mask' },
  'product.category.spot_treatment': { id: 'Spot Treatment', en: 'Spot Treatment' },
  'product.category.eye_cream': { id: 'Eye Cream', en: 'Eye Cream' },
  'product.category.lip_care': { id: 'Lip Care', en: 'Lip Care' },
  'product.skinFilter.all': { id: 'Semua Kulit', en: 'All Skin' },
  'product.loadError': { id: 'Gagal memuat produk', en: 'Failed to load products' },

  // ── Photo Progress remaining ──
  'progress.photoTooBig': { id: 'Foto terlalu besar. Max 2MB.', en: 'Photo too large. Max 2MB.' },
  'progress.uploadFailed': { id: 'Upload gagal. Coba lagi nanti.', en: 'Upload failed. Try again later.' },
  'progress.uploadSuccess': { id: 'Foto berhasil diupload!', en: 'Photo uploaded successfully!' },
  'progress.loadError': { id: 'Gagal memuat foto', en: 'Failed to load photos' },

  // ── Routine log remaining ──
  'routine.loadError': { id: 'Gagal memuat routine', en: 'Failed to load routine' },
};
